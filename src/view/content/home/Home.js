...
const Home = () => {
  ...
  const allText = useRef(
    `你可以编写一个这样的js配置来生成一个页面（大概需要花费您一分钟的时间，请耐心看完）
     全部配置请查看在线文档：https://betteryourself-tcy.github.io/json-page-markdown/， 也可从代码UsePage.js组件中查看
    const pageConfig = {
      // 接口的配置
      pageRequestUrl: {
        curdUrl: '/usePage/',
  ...
  const setTextFun = () => {
    ...
    timer.current = setInterval(() => {
      const strItem = allText.current.substring(index, index + 1)
      if (!strItem) {
      ...
      pageShowText.current += strItem
      index++

      textDom.current.scrollTop = textDom.current.scrollHeight
      setAgainRender([])
    }, 20)
  }
